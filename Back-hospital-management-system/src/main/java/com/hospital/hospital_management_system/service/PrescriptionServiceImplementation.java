...
public class PrescriptionServiceImplementation implements PrescriptionService {
    ...
    public PrescriptionDTO createPrescription(String doctorEmail, Long appointmentId, PrescriptionDTO dto) {
        ...
        if (dto.getMedicines() != null && !dto.getMedicines().isEmpty()) {
            for (PrescriptionMedicineDTO medicineDTO : dto.getMedicines()) {
                MedicineMaster medicineMaster = medicineMasterRepository.findById(medicineDTO.getMedicineId()).orElseThrow(() -> new MedicineNotFoundException("Medicine with ID " + medicineDTO.getMedicineId() + " not found."));
    ...
    public ResponseDTO deletePrescription(Long prescriptionId) {
        ...
        return new ResponseDTO(null, "Prescription Successfully added");
    }
    ...
    public PrescriptionMedicineDTO updateMedicine(Long prescriptionId, Long prescriptionMedicineId, PrescriptionMedicineDTO dto) {
        PrescriptionMedicine medicine = prescriptionMedicineRepository.findById(prescriptionMedicineId).orElseThrow(() -> new MedicineNotFoundException("No such Medicine found Exception"));

        if (!medicine.getPrescription().getPrescriptionId().equals(prescriptionId)) {

            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Medicine doesn't belong to this prescription");
        }
    ...
    @Override
    public PrescriptionDTO getDoctorPrescriptionByAppointmentId(String doctorEmail, Long appointmentId) {
        appointmentRepository.findByDoctorUserEmailAndAppointmentId(doctorEmail, appointmentId).orElseThrow(() -> new AccessDeniedException("Access denied"));

        return getPrescriptionByAppointmentId(appointmentId);
    }

    @Override
    public PrescriptionDTO getPatientPrescriptionByAppointmentId(String patientEmail, Long appointmentId) {
        Appointment appointment = appointmentRepository.findById(appointmentId).orElseThrow(() -> new AppointmentNotFoundException("No such appointment"));
    ...
    public Page<PrescriptionDTO> getPrescriptionsForPatient(String patientEmail, int page, int size) {
        Patient patient = patientRepo.findByUserEmail(patientEmail).orElseThrow(() -> new PatientNotFoundException("No Patient Found)"));
