...
import { registerPatientDetails, updatePatientDetails, getProfile } from '../api/patientApi';
import { getSessionItem, setSessionItem, removeSessionItem } from '../../utils/sessionStorage';

const BLOOD_GROUPS = ['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'];

export default function CompleteProfileModal({ open, onClose, onSuccess }) {
    ...
    try {
      ...
      try {
        ...
        if (err.response && (err.response.status === 409 || err.response.status === 400)) {
          await updatePatientDetails({ ...form, email });
